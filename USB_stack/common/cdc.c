...
#pragma udata usb_data cdc_acm_in_buffer
...
#pragma udata usb_data2 cdc_In_buffer
#pragma udata usb_data2 cdc_Out_buffer
BYTE cdc_In_buffer[CDC_BUFFER_SIZE];
BYTE cdc_Out_buffer[CDC_BUFFER_SIZE];

#pragma udata usb_data3 cdc_In_bufferA
#pragma udata usb_data3 cdc_In_bufferB
#pragma udata usb_data3 cdc_Out_bufferA
#pragma udata usb_data3 cdc_Out_bufferB
...
BYTE putPARTARRAYUSBUSART(BYTE *bdata, BYTE length) // JTR2 added reduced overhead
{
    while ((Inbdp->BDSTAT & UOWN)) {
        if (!TestUsbInterruptEnabled()) {
            FAST_usb_handler();
